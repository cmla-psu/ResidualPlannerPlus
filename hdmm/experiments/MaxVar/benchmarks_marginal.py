...
def DimKKrons_small(workloads, k=1,size=5000):
    blocks = workloads
    base = [workload.Total(W.shape[1]) for W in blocks]
    d = len(blocks)
...
def DimKKrons_kway_small(workloads, k=1,size=5000):
    #modified yingtai's kway code
    blocks = workloads
    base = [workload.Total(W.shape[1]) for W in blocks]
    d = len(blocks)
    ...
    for j in range(0,k+1):
        for attr in itertools.combinations(range(d), j):
            subs = [blocks[i] if i in attr else base[i] for i in range(d)]
            tmp = reduce(lambda x,y: x*y, [blocks[i].shape[1] for i in attr], 1)
            W = workload.Kronecker(subs)
            if tmp <= size:
