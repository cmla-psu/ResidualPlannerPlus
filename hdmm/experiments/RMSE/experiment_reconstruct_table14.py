...
class attributeData:
    ...
    def get_time_for_n(self, n):
        ...
        return times.mean(),2*np.sqrt(times.var())
...
def run_table_hdmm(attribute_Data,dims=[0,1,2,3],inputs=[2,4,8,16,32,64]):
    for n in inputs:
        d = 5
        domain = [n] * d
        ns=tuple(domain)
        ...
        t1 = time.time()
        lossout = np.sqrt(loss / W.shape[0])
        attribute_Data.add_data(n,t1-t0,lossout)
        print("-----------------------------------------")
        print("n,times of reconstruct ,loss")
        line = '%d, %.10f, %.10f' % (n, t1 - t0,lossout)
        print(line)


if __name__ == '__main__':
    fileName='tablehdmm14.csv'
    ...
    print("-----------------------------------------")
    print("n,avgtimes,vartimes,avgloss,varloss")
    for n in inputs:
