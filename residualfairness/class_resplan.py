...
def find_var_sum_cauchy_computational(var, pcost):
    ...
    c = 1
    size = len(var)
    x = cp.Variable(size, pos=True)

    obj = cp.Minimize(cp.sum(cp.multiply(var, cp.inv_pos(x))))
    constraints = [cp.sum(cp.multiply(pcost, x)) <= c]
    ...
    return 1 / x.value, obj.value


def find_var_sum_cauchy(vec_v, vec_p):
    """Solve the sum-of-variance optimization problem.

    min sum(v @ sigma^2)
    s.t. sum(p @ 1/sigma^2) == 1
    """
    T = np.sum(np.sqrt(vec_v * vec_p)) ** 2
    sigma_square = np.sqrt(vec_p / vec_v * T)
    return sigma_square, T
...
class ResidualPlanner:
    ...
    def __init__(self, domains):
        ...
        #NEW defined a list for easier access the marginal and submarginal sizes
        self.subset_indices = {}
        pass
    ...
    def input_mech(self, att, var_bound=1.0):
        ...
        for subset in att_subsets:
            if subset not in self.res_dict:
                #NEW
                self.subset_indices[subset] = len(self.subset_indices)
                #
    ...
    def get_coeff_maxvar(self, weight=None):
        if weight is None:
        ...
        for mech_att in self.mech_dict.keys():
            ...
            for res_att in subsets:
                var_coeff_list.append(self.var_coeff[mech_att, res_att] * weight[mech_att]**2)
    ...
    #TEST
    def get_coeff_sum_var_unweighted(self, weight=None):
        pcost_coeff_list = []
        if weight is None:
        ...
        for mech_att in self.mech_dict.keys():
            ...
            for res_att in subsets:
                idx = res2id[res_att]
                var_coeff[idx] += self.var_coeff[mech_att, res_att] * weight[mech_att]**2
        return var_coeff, pcost_coeff


    def get_coeff_sum_var(self, weight=None):
        """

        weight: the weight for marginal.
            weight[(1, 2)] = 2 means the weight for the marginal on (1, 2) is 2.
        """
        pcost_coeff_list = []
        if weight is None:
            weight = defaultdict(lambda: 1.0)
        ...
        for mech_att in self.mech_dict.keys():
            ...
            for res_att in subsets:
                ...
                #print(res_att)
                #print(idx)
                #print(self.var_coeff[mech_att, res_att])
                #print("------------------")
                #var_coeff[idx] += self.var_coeff[mech_att, res_att] * num_of_queries * weight[mech_att]**2
                #NOTE: NUM_QUERIES IS IGNORED TO GET UNWEIGHTED CELL VAR
                var_coeff[idx] += self.var_coeff[mech_att, res_att] * num_of_queries * weight[mech_att]**2
        return var_coeff, pcost_coeff
    #OLD ONE
    '''
    def selection(self, choice='sumvar'):
        ...
        return obj
    '''
    #NEW before var_coeff is passed to the optimization problem it is divided by all the attribute size of A 
    #to cancel out the product attribute size coefficient in the sum cell_variance * product attribute sizes in A
    #to get sum cell_variance
    def selection(self, choice='sumvar'):
        if choice == 'sumvar':
            var_coeff, pcost_coeff = self.get_coeff_sum_var()
            #sigma_square, obj = find_var_sum_cauchy_computational(var_coeff, pcost_coeff)
            sigma_square, obj = find_var_sum_cauchy(var_coeff, pcost_coeff)
        elif choice == 'maxvar':
            coeff, A, b = self.get_coeff_maxvar()
            sigma_square, obj = find_var_max(coeff, A, b)
    ...
    def print_variances(self):
        for att, mech in self.mech_dict.items():
            var = mech.output_variance()
            print(f"Variance for marginal {att}: {var}")

    #NEW
    def print_weighted_variances(self):
        for att, mech in self.mech_dict.items():
            weighted_variance = mech.output_variance() * mech.get_num_queries()
            print(f"Variance for marginal {att}: {weighted_variance}")

def test_Adult():
    #domains = [10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10]
    #domains = [100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]
    domains = [85, 9, 100, 16, 7, 15, 6, 5, 2, 100, 100, 99, 42, 2]
    #domains = [32, 33, 33, 32, 31, 30, 32, 31, 34, 30, 38, 39, 31, 33]
    #domains = [3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2]
...
def test_Adult():
    ...
    #domains = [100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100]
    domains = [85, 9, 100, 16, 7, 15, 6, 5, 2, 100, 100, 99, 42, 2]
    #domains = [85, 9, 100, 16, 7, 15, 6, 5, 2, 100, 100, 99, 42, 50]
    col_names = ['age', 'workclass', 'fnlwgt', 'education-num', 'marital-status',
       'occupation', 'relationship', 'race', 'sex', 'capital-gain',
       'capital-loss', 'hours-per-week', 'native-country', 'income>50K']
    system = ResidualPlanner(domains)
    ...
    att = tuple(range(len(domains)))
    total = 0
    remove_marginal = (0,13)  # Marginal to remove

    for i in range(0, 3):
        subset_i = list(itertools.combinations(att, i))
        print("Num of " + str(i) + "-way marginals: ", len(subset_i))
        for subset in subset_i:
            if subset != remove_marginal:  # Check to skip the marginal to remove
                system.input_mech(subset, 1**(3-i))
                cur_domains = [domains[c] for c in subset]
                total += np.multiply.reduce(cur_domains)

    # Add a specific 2-way marginal
    add_marginal = (0, 1, 2, 13)  # Specific marginal to add
    system.input_mech(add_marginal, 1)  # Assuming the same weight as others
    cur_domains = [domains[c] for c in add_marginal]
    total += np.multiply.reduce(cur_domains)
    print("Added specific 2-way marginal: ", add_marginal)
...
if __name__ == '__main__':
    ...
    system.reconstruct_covariance() 

    print("Cell Variance-----------------------------")
    system.print_variances()
    #print("Weighted V -------------------------------")
    #system.print_weighted_variances()
